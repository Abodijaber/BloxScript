...
local Worlds = {
    ...
    ["World 3"] = {FarmMobs={"Sky Pirate"}, CollectItems={"Sky Coins"}, Teleports={Vector3.new(2000,50,0)}, Islands={"Kitsuni","Volcano","Mirage"}}
}
...
local function fish()
    while AutoFish do
        print("Fishing...") -- ضع هنا كود الصيد حسب لعبتك
        wait(3)
    end
end
...
FeaturesFrame.Size = UDim2.new(0,150,1,0)
FeaturesFrame.Position = UDim2.new(0,0,0,0)
FeaturesFrame.BackgroundColor3 = Color3.fromRGB(35,35,35)

local featuresListLayout = Instance.new("UIListLayout", FeaturesFrame)
...
-- إنشاء أزرار المميزات
local featuresOptions = {
    {name="AutoFarm", var=function() return AutoFarm end, func=function(val) AutoFarm=val; if val then spawn(function() farmMobs(Worlds[CurrentWorld]) end) end end},
    {name="AutoFish", var=function() return AutoFish end, func=function(val) AutoFish=val; if val then spawn(fish) end end},
    {name="AutoCollect", var=function() return AutoCollect end, func=function(val) AutoCollect=val; if val then spawn(function() collectItems(Worlds[CurrentWorld]) end) end end},
    {name="InfiniteJump", var=function() return InfiniteJump end, func=function(val) InfiniteJump=val end}
}

for _,opt in ipairs(featuresOptions) do
    local btnContainer = Instance.new("Frame", FeaturesFrame)
    btnContainer.Size = UDim2.new(1,0,0,40)
    createToggle(btnContainer,opt.name,opt.var,opt.func)
end

-- Teleport للعالم الثالث (جزر)
local IslandFrame = Instance.new("Frame", MainFrame)
IslandFrame.Size = UDim2.new(0,150,0,150)
IslandFrame.Position = UDim2.new(0.5,10,0,60)
IslandFrame.BackgroundColor3 = Color3.fromRGB(45,45,45)
IslandFrame.Visible = false

local function createIslandButton(name,posY)
    ...
    btn.TextSize = 16
    btn.MouseButton1Click:Connect(function()
        local world = Worlds["World 3"]
        for _, islandName in ipairs(world.Islands) do
            if islandName == name then
                teleportTo(Vector3.new(3000,50,0)) -- ضع هنا موقع الجزيرة الصحيح
                IslandFrame.Visible = false
            end
        end
    end)
...
for i,islandName in ipairs(Worlds["World 3"].Islands) do
    createIslandButton(islandName,(i-1)*40)
end
